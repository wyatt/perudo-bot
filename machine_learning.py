...
class Population:
    ...
    def select_parents(self):
        total_fitness = sum(model.fitness for model in self.models)
        if total_fitness == 0:
            return random.sample(self.models, 2)
        probabilities = [model.fitness / total_fitness for model in self.models]
...
class Player():
    ...
    def state(self):
        ...
        dice_in_hand = self.dice
        # Pad dice_in_hand with 0s
        dice_in_hand += [0] * (5 - len(dice_in_hand))

        previous_bets = self.game.previous_bets
        is_my_turn = 1 if self == self.game.current_player else 0

        printd("Previous bets length ", previous_bets)

        return [count, value, 1 if palifico else 0, total_dice, num_dice_in_hand, *dice_in_hand, is_my_turn] + previous_bets
...
input_shape = 27  # Adjust based on your state representation
output_shape = 3  # Adjust based on your action space (30 possible bets + 1 for Dudo)
...
def evaluate_model(model, generation, model_index, epsilon):
    ...
    print(f"\nGeneration {generation + 1}/{NUM_GENERATIONS}, Model {model_index + 1}/{POPULATION_SIZE} Complete")
    print(f"Total Fitness: {model.fitness}")
...
def train_model():
    for generation in range(NUM_GENERATIONS):
        ...
        print(f"Average fitness: {sum(model.fitness for model in population.models) / POPULATION_SIZE}")
        ...
        if generation % 5 == 0:  # Changed from 10 to 5
            ...
            best_model.model.save(f'perudo_model_gen_{generation}.h5')
            print(f"Checkpoint saved: perudo_model_gen_{generation}.h5")
